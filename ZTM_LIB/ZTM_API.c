...
/*********************************************************************************************************
** 函数名称:  ZTM_Disable_Touch
** 函数功能:  在开机命令中将触摸屏响应类型设置为无效配置，并将触屏伴音关闭，系统启动后触摸屏进入静默状态，指令发送后可掉电保存  
              AA 96 00 18 AA 70 00 00 CC 33 C3 3C AA E0 03 00 CC 33 C3 3C AA E0 04 00 CC 33 C3 3C CC 33 C3 3C
** 输入参数:  无
** 输出参数:  无
** 返 回 值:  无
*********************************************************************************************************/
void ZTM_Disable_Touch (void)
{
    __SendHead();                                                       /* 发送帧头                     */
    
    __SendDByte(0x96);                                                  /* 发送命令                     */
    __SendDByte(0x00);
    __SendDByte(0x18);                                                  /* 发送参数                     */
    __SendDByte(0xAA);
    __SendDByte(0x70);
    __SendDByte(0x00);
    __SendDByte(0x00);
    ...
    __SendDByte(0xE003);                                                /* 发送命令                     */
    
    __SendDByte(0x00);                                                  /* 发送参数                     */
